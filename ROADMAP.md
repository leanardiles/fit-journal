...
|---|---|---|---|
| Create Cloudflare email (`info@fit-journal.com`) | 🟠 P1 | XS | Set up Cloudflare Email Routing on the `fit-journal.com` domain to forward `info@fit-journal.com` → personal inbox. Needed so the forgot-password page's contact address is live (and useful generally). DNS is already on Cloudflare, so it's a few clicks + verifying the destination address. |
| Add backend test gate to CI | 🟠 P1 | M | Write pytest tests for the FastAPI backend (auth, key endpoints), add a test step before deploy in the GitHub Actions workflow so broken code can't ship. Prerequisite for safely enabling auto-deploy on push. A smoke test that exercises `verify_password` would have caught the bcrypt-on-Lambda break. |
...
| Build forgot-password page + route | 🟡 P2 | XS | The login page links to `/web/forgot-password`; create the info page (reuses auth card styling; points to `info@fit-journal.com`) and add the `@app.get("/web/forgot-password")` route in `main.py`. Don't deploy the login link until this exists, or the link 404s. |
