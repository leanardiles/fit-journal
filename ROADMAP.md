...
| Security hygiene quick wins | 🟡 P2 | XS | Independent of API paths, can be done anytime. Verify CORS isn't wildcard-with-credentials; set DB engine `echo=False` in production (currently logs queries + hashes); audit git history for a committed `SECRET_KEY` (rotate if exposed); enable Dependabot. The SECRET_KEY audit and echo=False are mild current exposures — worth doing sooner rather than later. |

| Consolidate duplicate login endpoints | 🟡 P2 | S | `/login` (web) and `/login/mobile` are 95% identical — same auth logic, differing only in cosmetic response fields (web returns `message`, mobile returns `token_type: "bearer"`). Merge into one `/login` returning a response both clients can parse (include `token_type`; mobile's `LoginResponse` requires it, web ignores it). Then update mobile `ApiConfig.kt` (`LOGIN = "login/mobile"` → `"login"`) and rebuild, and remove `/login/mobile`. Do *after* mobile is confirmed working against production (so any breakage is isolated to this change, not tangled with the prod cutover). Low effort; removes drift risk between two near-identical endpoints. |
| Introduce API versioning before public launch | 🟠 P1 | M | Prefix the JSON API routes with a version (e.g. `/v1/login`, `/v1/exercises`) so future backend changes don't break old installed app versions once the app is on the Play Store (users running an outdated app hit a stable, versioned contract). Cross-cutting: touches every JSON endpoint in `main.py`, the web `api.js` calls, the mobile `ApiConfig.kt` endpoints + Retrofit paths, and all backend tests. The `/web/*` HTML page routes stay unversioned (they're server-rendered, not a client API contract). Pair with a backward-compatibility discipline: additive changes only (add fields, don't rename/remove), so old clients keep working. Cheapest to do now (pre-launch, no installed apps to keep compatible yet) than after real users exist. Do as a dedicated focused effort — not interleaved with other in-flight work — since a half-applied re-pathing breaks things. |
| Authorization test sweep | 🟠 P1 | S | Write tests proving user A can't access/modify user B's exercises, routine, workout logs, and next-workout selections — confirms `verify_user_access` coverage or surfaces remaining IDOR gaps. Watch resource-id endpoints (e.g. `/workout/logs-by-sessions`). **Do after API versioning** so the tests are written once against the final versioned paths, avoiding a rewrite. |
